...
var sSharedCHM= moniker+"gday.chm::/images\\";
...
function callPopup(eventSrc) {
  ...
  if (iTOPIC==-1) return;
  sParamTOPIC = EncodeID(eH.substring((iTOPIC+6),eH.length));  		// extracts the topic for item2

  if (document.hhPopup) document.hhPopup.outerHTML = "";	// if hhPopup object exists, clears it

  document.body.insertAdjacentHTML("beforeEnd", "<object id='hhPopup'"+ sActX_HH + "STYLE='display:none'></object>");
  document.hhPopup.TextPopup(sParamTOPIC, FontFace,10,5,-1,-1);
}


//*** EncodeID******************************************************************************
// formats a string based on ID passed by topic_id 
// called from: callPopup()


function EncodeID(topic_id) {
  if (topic_id=="IDH_DATUM") topic_id = "The physical realisation of the reference system used to describe the earth's size and shape, its point of origin and orientation.";
  else if (topic_id=="IDH_GEOGRAPHIC") topic_id = "A set of coordinates (expressed in terms of Latitude and Longitude) representing a point on the earth's surface in relation to the equator and Greenwich meridian.";  
  else if (topic_id=="IDH_PROJECTION") topic_id = "A set of coordinates (expressed in terms of Easting, Northing and Zone) derived by mathematically transforming geographical coordinates on a spheroid to a flat, planar surface.";  
  else if (topic_id=="IDH_NTV2") topic_id = "National Transformation version 2: the adopted file format for structuring a regularly gridded array of distortion shifts and their associated accuracy."
  else if (topic_id=="IDH_AGD") topic_id = "Australian Geodetic Datum: A locally defined datum based on a reference ellipsoid (ANS) that best fits the geoid in the Australian region.";  
  else if (topic_id=="IDH_GDA") topic_id = "Geocentric Datum of Australia: A datum based on a reference ellipsoid (GRS80) that has its centre coincident with the earth's centre of mass, producing a best fit for the entire globe.";  
  else if (topic_id=="IDH_AMG") topic_id = "Australian Map Grid: Coordinates on the AMG are derived from a Transverse Mercator projection of latitudes and longitudes on the AGD.";  
  else if (topic_id=="IDH_MGA") topic_id = "Map Grid of Australia: Coordinates on the MGA are derived from a Transverse Mercator projection of latitudes and longitudes on the GDA.";  
  else if (topic_id=="IDH_ANS") topic_id = "Australian National Spheroid: The reference ellipsoid that best fits the geoid in the Australia region and is used to base the AGD.";  
  else if (topic_id=="IDH_GRS80") topic_id = "Geodetic Reference System of 1980: The GRS80 is the reference ellipsoid used to base the Geodetic Datum of Australia. For most practical purposes, the GRS80 is synonymous with the WGS84 reference frame and thereby enables GPS coordinates to be directly compatible with spatial products based on a GDA94 coordinate system.";
  else if (topic_id=="IDH_FORWARD") topic_id = "Transformation of coordinates from AGD to GDA.";
  else if (topic_id=="IDH_REVERSE") topic_id = "Transformation of coordinates from GDA to AGD.";
  return topic_id;  
}
